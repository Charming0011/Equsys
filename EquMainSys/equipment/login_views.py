...
def user_logout(request):
    ...
    return render(request, 'Login.html', {'msg': '登出成功！'})
...
def stu_login(request):
    if request.method=='GET':
        ...
        if user:
            ...
            stu=Stu.objects.filter(stuser=userid).update(lastime=d)
            #存入近期登陆
            rec=Recent.objects.create(userid=userid,username=username,logindate=d)
            #设置关闭浏览器清除session
            request.session.set_expiry(0)
            return redirect('/submaninfo')
        else:
            return render(request,'stulogin.html',{'msg':'用户名或密码错误！'})

def submaninfo(request):
    username=request.session.get('username','')
    if not username:
        return render(request,'stulogin.html',{'msg':'请登录！'})
    if request.method=='GET':
